...
window.addEventListener('load', () => {
	fetch(`fetchYears/bills/billDate`)
	.then(r => r.text())
	.then(text => {
...
const logAsRecent = (month, year, link) => {
	...
	let sales = recentReports.sales
	const monthArray = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
	let title = `${monthArray[month - 1]} - ${year}`

	sales.unshift({title: `${title}`, link: `${link}`})
	if (sales.length > 3) {
		sales.pop()
	}
	recentReports.sales = sales
...
const loadRecentReports = () => {
	...
	if (localStorage.recentReports == undefined || JSON.parse(localStorage.recentReports).sales.length == 0) {
